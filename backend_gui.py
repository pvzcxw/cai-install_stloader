        ...
        try:
            ...
            if self.is_steamtools():
                ...
                if not is_st_auto_update_mode:
                    ...
                    if manifest_files:
                        for f in manifest_files: shutil.copy2(f, st_depot)
                        self.log.info(f"[{source_name}] 已复制 {len(manifest_files)} 个清单到SteamTools目录。")
                    else: self.log.info(f"[{source_name}] 未找到 .manifest 文件。")
                else:
                    self.log.info(f"[{source_name}] ST自动更新模式: 已跳过安装 .manifest 文件。")
