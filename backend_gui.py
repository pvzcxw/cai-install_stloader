...
# --- MODIFICATION: client is no longer global ---
# client = httpx.AsyncClient(...) # This line is removed
...
class GuiBackend:
    ...
    # --- Steam & Unlocker Detection (no changes) ---
    ...
    async def check_github_api_rate_limit(self, client: httpx.AsyncClient, headers: dict):
        ...
        try:
            ...
            rate, remaining = r.json().get('rate', {}), 0
            if rate: remaining = rate.get('remaining', 0)
            if remaining == 0:
                self.log.warning(f"GitHub API请求数已用尽，将在 {rate.get('reset', 0)} 重置。")
                return False
    ...
    # --- Manifest and File Processing (MODIFIED) ---
    ...
    async def get_manifest_from_github(self, client: httpx.AsyncClient, sha: str, path: str, repo: str, app_id: str, all_manifests: List[str]):
        is_st_lua_only_mode = self.is_steamtools() and self.app_config.get("steamtools_only_lua", False)
        if path.endswith('.manifest') and is_st_lua_only_mode:
            self.log.info(f"ST-LUA模式: 已跳过清单文件: {path}"); return []
        ...
        if path.endswith('.manifest'):
            ...
            self.log.info(f'Lua脚本已保存 (替换): {stplug/Path(path).name}')
    ...
    async def _process_zip_based_manifest(self, client: httpx.AsyncClient, app_id: str, download_url: str, source_name: str):
        try:
            ...
            if not manifests and not luas:
                self.log.warning(f"[{source_name}] 压缩包中未找到任何 .manifest 或 .lua 文件。"); return False
            ...
            if self.is_steamtools():
                ...
                if luas:
                    ...
                    self.log.info(f"[{source_name}] 已复制并替换 {len(luas)} 个脚本到SteamTools目录。")
                if not is_st_lua_only_mode:
                    self.log.info(f'[{source_name}] 按SteamTools标准模式安装清单文件。')
                    st_depot = self.steam_path / 'config' / 'depotcache'; st_depot.mkdir(parents=True, exist_ok=True)
                    if manifests:
                        for f in manifests: shutil.copy2(f, st_depot)
                        self.log.info(f"[{source_name}] 已复制 {len(manifests)} 个清单到SteamTools目录。")
                    else: self.log.info(f"[{source_name}] 未找到 .manifest 文件。")
                else: self.log.info(f"[{source_name}] ST-LUA模式: 已跳过安装 .manifest 文件。")
            else:
                ...
                else: self.log.warning(f"[{source_name}] 在GreenLuma/标准模式下未找到可安装的 .manifest 文件。")
            return True
        except Exception as e:
