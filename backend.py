...
class CaiBackend:
    ...
    async def _get_dlc_ids(self, appid: str) -> List[str]:
        """Gets all DLC IDs for a given AppID."""
        data = await self._get_steamcmd_api_data(appid)
        if not data:
            return []
        info = data.get("data", {}).get(str(appid), {})
        dlc_str = (info.get("common", {}).get("listofdlc", "")
                   or info.get("extended", {}).get("listofdlc", ""))
    ...
    async def _get_depots(self, appid: str) -> List[Dict]:
        """Gets all depot information for a given AppID."""
        data = await self._get_steamcmd_api_data(appid)
        if not data:
            return []
        info = data.get("data", {}).get(str(appid), {})
        ...
        for depot_id, depot_info in depots.items():
            ...
            out.append({"depot_id": depot_id, "size": size, "dlc_appid": dlc_appid})
        return out
    ...
    async def _process_zip_manifest_generic(self, app_id: str, download_url: str, source_name: str, add_all_dlc: bool = False) -> bool:
    ...
    async def process_printedwaste_manifest(self, app_id: str, add_all_dlc: bool = False) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://api.printedwaste.com/gfk/download/{app_id}', "SWA V2 (printedwaste)", add_all_dlc)

    async def process_cysaw_manifest(self, app_id: str, add_all_dlc: bool = False) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://cysaw.top/uploads/{app_id}.zip', "Cysaw", add_all_dlc)

    async def process_furcate_manifest(self, app_id: str, add_all_dlc: bool = False) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://furcate.eu/files/{app_id}.zip', "Furcate", add_all_dlc)

    async def process_assiw_manifest(self, app_id: str, add_all_dlc: bool = False) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://assiw.cngames.site/qindan/{app_id}.zip', "CNGS (assiw)", add_all_dlc)

    async def process_steamdatabase_manifest(self, app_id: str, add_all_dlc: bool = False) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://steamdatabase.s3.eu-north-1.amazonaws.com/{app_id}.zip', "SteamDatabase", add_all_dlc)
    ...
    async def process_github_manifest(self, app_id: str, repo: str, add_all_dlc: bool = False) -> bool:
        github_token = self.config.get("Github_Personal_Token", "")
        headers = {'Authorization': f'Bearer {github_token}'} if github_token else None
