...
class CaiBackend:
    ...
    async def _get_from_mirrors(self, sha: str, path: str, repo: str) -> bytes:
        ...
        for url in urls:
            try:
                ...
                self.log.error(f'下载失败: {path} (来自 {url}) - 状态码: {r.status_code}')
            except httpx.RequestError as e:
                self.log.error(f'下载失败: {path} (来自 {url}) - 错误: {e}')
    ...
    async def _process_zip_manifest_generic(self, app_id: str, download_url: str, source_name: str) -> bool:
        ...
        try:
            ...
            if self.is_steamtools() and lua_files:
                ...
                for f in lua_files: shutil.copy2(f, stplug_path / f.name); self.log.info(f'已复制lua脚本: {f.name}')
            else:
                self.log.info(f'为GreenLuma处理来自 {source_name} 的文件。')
    ...
    async def process_github_manifest(self, app_id: str, repo: str) -> bool:
        ...
        for path in manifest_paths:
            ...
            self.log.info(f"已保存清单: {filename}")
        ...
        if self.is_steamtools():
            ...
            for path in lua_paths: (stplug_path / Path(path).name).write_bytes(downloaded_files[path])
    ...
    async def find_appid_by_name(self, game_name: str) -> List[Dict]:
        try:
            r = await self.client.get(f'https://steamui.com/loadGames.php?search={game_name}')
            r.raise_for_status()
