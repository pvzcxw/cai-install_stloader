...
class CaiBackend:
    ...
    def parse_lua_file(self, lua_file_path: str) -> Tuple[Dict, Dict]:
    ...
    async def _process_zip_manifest_generic(self, app_id: str, download_url: str, source_name: str) -> bool:
        ...
        try:
            ...
            if self.is_steamtools() and lua_files:
                self.log.info(f'为SteamTools处理来自 {source_name} 的文件。')
                ...
                if not self.use_st_auto_update:
                ...
                for f in lua_files: 
                    shutil.copy2(f, stplug_path / f.name)
                    self.log.info(f'已复制lua脚本: {f.name}')
            else: 
                if not manifest_files:
                    self.log.warning(f"在来自 {source_name} 的压缩包中未找到 .manifest 文件。")
                    return False
                ...
                for lua in lua_files:
                    depots, _ = self.parse_lua_file(str(lua))
                    if depots: all_depots.update(depots)
                if all_depots:
    ...
    async def process_github_manifest(self, app_id: str, repo: str) -> bool:
        ...
        files_to_process = r2_json['tree']
        
        # Filter out .manifest files BEFORE downloading if ST auto-update is enabled
        if self.is_steamtools() and self.use_st_auto_update:
            self.log.info("已启用SteamTools自动更新，将跳过下载 .manifest 文件。")
            files_to_process = [item for item in files_to_process if not item['path'].endswith('.manifest')]
        
        if not files_to_process:
            self.log.warning(f"在 {repo}/{app_id} 中没有找到需要下载的文件。")
            return True # Still considered a success if we just don't need anything.

        tasks = [self._get_from_mirrors(sha, item['path'], repo) for item in files_to_process]
        # --- END OF MODIFICATION ---
        
        try:
            downloaded_contents = await asyncio.gather(*tasks)
            downloaded_files = {item['path']: content for item, content in zip(files_to_process, downloaded_contents)}
        except Exception as e:
            self.log.error(f"下载文件失败，正在中止对 {app_id} 的处理: {e}")
            return False
        
        manifest_paths = [p for p in downloaded_files if p.endswith('.manifest')]
        lua_paths = [p for p in downloaded_files if p.endswith('.lua')]
        ...
        if self.is_steamtools():
            ...
            # This block now only runs if use_st_auto_update is FALSE,
            # because if it's TRUE, manifest_paths will be empty.
            if manifest_paths:
            ...
            async with aiofiles.open(lua_filepath, mode="w", encoding="utf-8") as lua_file:
                ...
                if key_vdf_path:
                    ...
                    for depot_id, info in depots_config.get('depots', {}).items():
                        await lua_file.write(f'addappid({depot_id}, 1, "{info["DecryptionKey"]}")\n')
                
                # This check remains correct. If auto-update is on, manifest_paths is empty.
                if not self.use_st_auto_update:
                    for manifest_file in manifest_paths:
                        match = re.search(r'(\d+)_(\w+)\.manifest', Path(manifest_file).name)
                        if match: await lua_file.write(f'setManifestid({match.group(1)}, "{match.group(2)}")\n')

            for path in lua_paths: 
                (stplug_path / Path(path).name).write_bytes(downloaded_files[path])
            self.log.info(f"已为SteamTools生成/复制解锁文件: {app_id}")
        else: # GreenLuma
            ...
            if key_vdf_path:
                depots_config = vdf.loads(downloaded_files[key_vdf_path].decode('utf-8'))
                await self.depotkey_merge(self.steam_path / 'config' / 'config.vdf', depots_config)
                gl_ids = list(depots_config.get('depots', {}).keys())
