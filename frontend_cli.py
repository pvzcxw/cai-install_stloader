...
def show_banner(backend: CaiBackend):
    ...
    log.info('Cai install XP版本：1.51b1')
    ...
    log.warning('vdf writer v2  已接入自研manifest2lua se')
...
async def main_flow(backend: CaiBackend):
    ...
    # 如果检测到是SteamTools，则询问用户解锁模式
    if backend.is_steamtools():
        while True:
            try:
                print(f"\n{Fore.YELLOW}检测到您正在使用 SteamTools。")
                choice = input(f"{Fore.GREEN}是否启用其自动更新清单功能？(y/n) (推荐): {Style.RESET_ALL}").lower().strip()
                if choice in ['y', 'yes', '是']:
                    backend.use_st_auto_update = True
                    # 询问是否固定版本
                    while True:
                        try:
                            print(f"\n{Fore.YELLOW}SteamTools 自动更新模式选项：")
                            print(f"{Fore.CYAN}  y: 固定版本 - 将当前清单的版本号写入解锁脚本，锁定游戏到该版本(D加密推荐选择)")
                            print(f"{Fore.CYAN}  n: 浮动版本 - 不固定清单版本号，由SteamTools自动获取最新版本(普通游戏推荐){Style.RESET_ALL}")
                            lock_choice = input(f"{Fore.GREEN}是否固定清单版本？(y/n) (默认n): {Style.RESET_ALL}").lower().strip()

                            if lock_choice in ['y', 'yes', '是']:
                                backend.st_lock_manifest_version = True
                                log.info("已选择【固定版本】模式。将把当前版本号写入脚本。")
                                break
                            elif lock_choice in ['n', 'no', '否', '']: # 默认选项
                                backend.st_lock_manifest_version = False
                                log.info("已选择【浮动版本】模式。将不指定版本号，由SteamTools自动更新。")
                                break
                            else:
                    ...
                    log.info("已禁用自动更新。将按照传统方式导入清单和脚本。")
                    break
                else:
                    log.error("无效输入，请输入 y 或 n。")
            except (EOFError, KeyboardInterrupt):
    ...
    if search_choice == 1:
        await handle_repo_selection(backend, input_items)
    elif search_choice == 2:
        await handle_github_search(backend, input_items)
    else:
        log.error("无效的选择，请输入1或2。")

async def handle_repo_selection(backend: CaiBackend, items: list):
    log = backend.log
    print(f"\n{Fore.YELLOW}请选择清单库：")
    repo_map = {
        1: ("SWA V2库", backend.process_printedwaste_manifest),
        2: ("Cysaw库", backend.process_cysaw_manifest),
        3: ("Furcate库", backend.process_furcate_manifest),
        4: ("CNGS库", backend.process_assiw_manifest),
        5: ("SteamDatabase库", backend.process_steamdatabase_manifest),
    }
    ...
    for item in items:
        ...
        if choice in repo_map:
            ...
            success = await backend.process_github_manifest(app_id, repo)
        else:
            log.error(f"无效的仓库选择: {choice}")
        log.info(f"--- AppID {app_id} 处理 {'成功' if success else '失败'} ---\n")

async def handle_github_search(backend: CaiBackend, items: list):
    log = backend.log
