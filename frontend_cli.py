...
def show_banner(backend: CaiBackend):
    ...
    log.info('Cai install XP版本：1.53p1')
    ...
    log.warning('vdf writer v2  已接入自研manifest2lua se  DLC检索入库by B-I-A-O感谢其技术支持')
...
async def main_flow(backend: CaiBackend):
    ...
    add_all_dlc = False # <--- NEW: Initialize dlc flag
    ...
    if backend.is_steamtools():
        ...
        # <--- NEW LOGIC START --->
        # Ask about adding all free DLCs
        while True:
            try:
    ...
    if search_choice == 1:
        # Pass the new flag to the handler
        await handle_repo_selection(backend, input_items, add_all_dlc)
    elif search_choice == 2:
        # Pass the new flag to the handler
        await handle_github_search(backend, input_items, add_all_dlc)
    else:
        log.error("无效的选择，请输入1或2。")

# <--- MODIFIED: Added add_all_dlc parameter --->
async def handle_repo_selection(backend: CaiBackend, items: list, add_all_dlc: bool):
    log = backend.log
    print(f"\n{Fore.YELLOW}请选择清单库：")
    # <--- MODIFIED: Pass add_all_dlc to backend functions --->
    repo_map = {
        1: ("SWA V2库", lambda app_id: backend.process_printedwaste_manifest(app_id, add_all_dlc)),
        2: ("Cysaw库", lambda app_id: backend.process_cysaw_manifest(app_id, add_all_dlc)),
        3: ("Furcate库", lambda app_id: backend.process_furcate_manifest(app_id, add_all_dlc)),
        4: ("CNGS库", lambda app_id: backend.process_assiw_manifest(app_id, add_all_dlc)),
        5: ("SteamDatabase库", lambda app_id: backend.process_steamdatabase_manifest(app_id, add_all_dlc)),
    }
    ...
    for item in items:
        ...
        if choice in repo_map:
            ...
            # <--- MODIFIED: Pass add_all_dlc --->
            success = await backend.process_github_manifest(app_id, repo, add_all_dlc)
        else:
            log.error(f"无效的仓库选择: {choice}")
        log.info(f"--- AppID {app_id} 处理 {'成功' if success else '失败'} ---\n")

# <--- MODIFIED: Added add_all_dlc parameter --->
async def handle_github_search(backend: CaiBackend, items: list, add_all_dlc: bool):
    log = backend.log
