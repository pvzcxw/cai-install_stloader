...
# --- MODIFICATION: New Dialog for Version Locking ---
class VersionLockDialog(tk.Toplevel):
    def __init__(self, parent, title="选择版本模式"):
        super().__init__(parent)
        self.transient(parent); self.title(title); self.parent = parent
        self.result = None # None means user cancelled
        self.grab_set()
    ...
    def create_body(self, master):
        ttk.Label(master, text="请选择SteamTools清单更新模式：", justify=LEFT, font=("", 11, 'bold')).pack(pady=(0,15))
        
        fixed_button = ttk.Button(master, text="✅ 固定版本", command=lambda: self.ok(lock=True))
        fixed_button.pack(fill=X, pady=5)
        ttk.Label(master, text="将当前版本号写入脚本，锁定此版本（D加密推荐）", wraplength=300, justify=LEFT).pack(fill=X, pady=(0, 10))
        
        float_button = ttk.Button(master, text="🔄 浮动版本", command=lambda: self.ok(lock=False))
        float_button.pack(fill=X, pady=5)
        ttk.Label(master, text="不写入版本号，由ST自动获取最新版本", wraplength=300, justify=LEFT).pack(fill=X, pady=(0, 10))

        return fixed_button

    def ok(self, lock: bool):
        self.result = lock
        self.cancel()
...
class CaiInstallGUI(ttk.Window):
    def __init__(self):
        super().__init__(themename="darkly", title="Cai Install XP v1.51b1 - GUI Edition")
    ...
    def start_processing(self):
        ...
        # --- MODIFICATION: Show version lock dialog ---
        ...
        if is_st_auto_update_mode:
            dialog = VersionLockDialog(self)
            if dialog.result is None: # User closed the dialog
                self.log.warning("操作已取消。")
                self.processing_lock.release()
                return
            self.backend.st_lock_manifest_version = dialog.result
            self.log.info(f"已选择版本模式: {'固定版本' if dialog.result else '浮动版本'}")
        # --- END OF MODIFICATION ---
    ...
    def show_about_dialog(self):
        messagebox.showinfo("关于", "Cai Install XP v1.51b1 - GUI Edition\n\n原作者: pvzcxw\nGUI重制: pvzcxw\n\n一个用于Steam游戏清单获取和导入的工具")
    ...
    def show_settings_dialog(self):
        ...
        st_lua_only_check = ttk.Checkbutton(frame, text="使用SteamTools进行清单更新 (仅下载LUA)", variable=st_lua_only_var, bootstyle="round-toggle")
